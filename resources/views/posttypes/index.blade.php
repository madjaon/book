                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <!-- <div style="display: flex; justify-content: space-between; align-items: center;"> -->
                            ...
                            Showing post types: {{ count($data) }}/{{ $data->total() }}

                            <a href="{{ URL::to('posttypes/create') }}" class="btn btn-default btn-xs pull-right margin-right-1">
                                ...
                                <tbody>
                                    @foreach($data as $value)
                                        <tr>
                                            ...
                                            <td>{{$value->id}}</td>
                                            <td>{{$value->name}}</td>
                                            <td><a id="status_{{ $value->id }}" onclick="updateStatus('posttypes', {{ $value->id }}, 'status')" style="cursor: pointer; text-decoration: none;" title="Click to change">{!! CommonOption::getStatus($value->status) !!}</a></td>
                                            <td class="hidden-sm hidden-xs hidden-md">{{$value->created_at}}</td>
                                            <td class="hidden-sm hidden-xs hidden-md">{{$value->updated_at}}</td>
                                            <td>
                                                {!! Form::open(array('url' => 'posttypes/' . $value->id, 'class' => '', 'data-toggle' => 'tooltip', 'title' => 'Delete')) !!}
                                            ...
                                            <td>
                                                <a class="btn btn-sm btn-success btn-block" href="{{ url('the-loai/' . $value->slug) }}" data-toggle="tooltip" title="Show" target="_blank">
                                                    <i class="fa fa-eye fa-fw" aria-hidden="true"></i> <span class="hidden-xs hidden-sm">Show</span>
                                                </a>
                                            </td>
                                            <td>
                                                <a class="btn btn-sm btn-info btn-block" href="{{ URL::to('posttypes/' . $value->id . '/edit') }}" data-toggle="tooltip" title="Edit">
