                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <!-- <div style="display: flex; justify-content: space-between; align-items: center;"> -->
                            ...
                            Showing ads: {{ count($data) }}/{{ $data->total() }}

                            <a href="{{ URL::to('ads/create') }}" class="btn btn-default btn-xs pull-right margin-right-1">
                            ...
                            <table class="table table-striped table-condensed data-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="hidden-sm hidden-xs hidden-md">EndDate</th>
                                ...
                                <tbody>
                                    @foreach($data as $value)
                                        <tr>
                                            ...
                                            <td><a id="status_{{ $value->id }}" onclick="updateStatus('ads', {{ $value->id }}, 'status')" style="cursor: pointer; text-decoration: none;" title="Click to change">{!! CommonOption::getStatus($value->status) !!}</a></td>
                                            <td class="hidden-sm hidden-xs hidden-md">{{$value->start_date}}</td>
                                            <td class="hidden-sm hidden-xs hidden-md">{{$value->end_date}}</td>
                                            <td class="hidden-sm hidden-xs hidden-md">{{$value->created_at}}</td>
                                            <td class="hidden-sm hidden-xs hidden-md">{{$value->updated_at}}</td>
                                            <td>
                                                {!! Form::open(array('url' => 'ads/' . $value->id, 'class' => '', 'data-toggle' => 'tooltip', 'title' => 'Delete')) !!}
                                            ...
                                            <td>
                                                <a class="btn btn-sm btn-info btn-block" href="{{ URL::to('ads/' . $value->id . '/edit') }}" data-toggle="tooltip" title="Edit">
