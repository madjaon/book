@php 
	...
	$type_main_id = old('type_main_id') ? old('type_main_id') : $data->type_main_id;
	$currentPostTagId = old('posttype_id') ? old('posttag_id') : CommonQuery::getArrayPostRelationId($data->id, 1);
	$currentPostTypeId = old('posttype_id') ? old('posttype_id') : CommonQuery::getArrayPostRelationId($data->id);
@endphp
  	...
  	<div class="table-responsive list-border">
	    <div class="list-box">
    		<div class="input-group">
				@if($posttypes)
					<input type="hidden" name="type_main_id" value="{{ $type_main_id }}" required>
					<ul class="todo-list">
						@foreach($posttypes as $key => $value)
							<li class="post-type-list">
								<input type="checkbox" name="posttype_id[]" value="{{ $value->id }}" class="posttype_id" id="posttype_id_{{ $value->id }}" onclick="checkPostType({{ $value->id }});" {{ CommonQuery::postTypeChecked($currentPostTypeId, $value->id) }} />
								<label for="posttype_id_{{ $value->id }}">{{ $value->name }}</label>
								<small class="label label-success primary" id="primary_{{ $value->id }}" style="{{ CommonQuery::postTypeCheckedDisplay($type_main_id, $value->id) }}"><i class="fa fa-key"></i> Primary</small>
								<small class="make_primary" id="make_primary_{{ $value->id }}" onclick="checkKey({{ $value->id }}, 'primary', 'type_main_id');" style="cursor: pointer; {{ CommonQuery::postTypeMakeDisplay($data->id, $type_main_id, $value->id) }}"> Primary</small>
							</li>
						@endforeach
					</ul>
...
<div class="form-group has-feedback row {{ $errors->has('posttag_id') ? ' has-error ' : '' }}">
	...
	<div class="col-md-9">
	  <div class="input-group">
	    @if($posttags)
			{!! Form::select('posttag_id[]', $posttags, $currentPostTagId, array('class' => 'form-control select2', 'multiple' => 'multiple', 'data-placeholder' => trans('forms.form_ph_tag'), 'style' => 'width: 100%;')) !!}
		@else
			<i>Chưa có dữ liệu</i>
		@endif
